...
export function SimpleBarChart({
  ...
  const barW = 16;
  const gap = 6;
  const svgW = Math.max(SCREEN_W - 48, data.length * (groupW + gap) + 32);
          ...
          {data.map((d, i) => {
            ...
            return (
              <React.Fragment key={`${d.label}-${i}`}>
                ...
                <Rect x={x} y={revY} width={barW} height={Math.max(revH, 2)} fill={revenueColor} rx={4} opacity={0.9} />
                {d.expenses > 0 && (
                  <Rect x={x + barW + 4} y={expY} width={barW} height={Math.max(expH, 2)} fill={expenseColor} rx={4} opacity={0.85} />
                )}
